    ...
    <div class="public-page-layout">
        ...
        <div class="content-area">
            ...
            <div v-if="commonActiveTab === 'details'">
                    <DetailsSection :payHisEntity="payHisEntity"  :loanDueEntity="loanDueEntity"  :fundsPaymentPlan="fundsPaymentPlan"/>
            </div>
            <!-- 审批历史 -->
...
/*** 非公共组件 */
import DetailsSection from '@/views/workstation/approval/bottom_menu/my_approvals/pending/loan/putoutConfirm/DetailsSection.vue';
...
export default {
    name: 'PutoutConfirmApprovalPage',
    components: {
        LoadingIndicator,
        HistoryTimeline,        
        DocumentList,
    ...
    methods: {
        ...
        getData(){
            let data =  {
                    pactNo: this.payHisEntity.pactNo,
                    applyNo: this.payHisEntity.applyNo,
                    payPactName: this.payHisEntity.payPactName,
                    cusName: this.payHisEntity.cusName,
                    productName: this.payHisEntity.productName,
                    pactAmt: this.payHisEntity.pactAmt,
                    dueChildAmtReally: this.payHisEntity.dueChildAmtReally,
                    dueChildAmtPayReally: this.payHisEntity.dueChildAmtPayReally,
                    payDueAmt: this.payHisEntity.payDueAmt,
                    repayTypeExpand: this.payHisEntity.repayTypeExpand || '',
                    dueBeginDate: this.payHisEntity.dueBeginDate || '',
                    term: this.payHisEntity.term,
                    vouType: this.payHisEntity.vouType,
                    repayDateSet: this.payHisEntity.repayDateSet,
                    repayDate: this.payHisEntity.repayDate,
                    loanType: this.payHisEntity.loanType,
                    purpose: this.payHisEntity.purpose,
                    applicationComments: this.payHisEntity.applicationComments,
                    platAccountNo: this.payHisEntity.platAccountNo,
                    platBankName: this.payHisEntity.platBankName,
                    platAccountName: this.payHisEntity.platAccountName,
                    repayAccountNo: this.payHisEntity.repayAccountNo,
                    repayBankName: this.payHisEntity.repayBankName,
                    repayAccountName: this.payHisEntity.repayAccountName,
                    pactId: this.payHisEntity.pactId,
                    putoutAccountName: this.payHisEntity.putoutAccountName,
                    putoutAccountNo: this.payHisEntity.putoutAccountNo,
                    applyId: this.payHisEntity.applyId,
                    dueEndDateShow: this.payHisEntity.dueEndDateShow,
                    rate: this.payHisEntity.rate,
                    overRate: this.payHisEntity.overRate,
                    dueEndDate: this.payHisEntity.dueEndDate,
                    putoutBankName: this.payHisEntity.putoutBankName,
                    cmpdRate: this.payHisEntity.cmpdRate,
                    dueId: this.payHisEntity.dueId,
                    rateType: this.payHisEntity.rateType,
                    overFloat: this.payHisEntity.overFloat,
                    icType: this.payHisEntity.icType,
                    termType: this.payHisEntity.termType,
                    cmpdFloat: this.payHisEntity.cmpdFloat,
                    payId: this.payHisEntity.payId,
                    dueAmt: this.payHisEntity.dueAmt
            };
            ...
            if(this.activeUserList.length > 0){
                ...
                this.$confirm(
                    ...
                    '提示',
                    {
                        ...
                        // 执行提交操作
                        // “选人后” 提交
                        const submitUrl = `/${this.$config.servers.loan}/due/loanDuePayHis/submitApprove`;
                        ...
                        api.getApprovalDetail(
                            ...
                            response => {
                                if (response && response.code === 0 && response.data && response.data.approveBtnList.length > 0) {
                                    ...
                                    resolve(this.payId);//委贷-支付审批，这里需要传payId
        ...
        fetchpayHisEntity(task_id, biz_id ) {
            ...
            .then(payId => {
                ...
                //（3）判断是否成功获取payId
                if (!payId) {
                    throw new Error('No payId found, skipping second API call');
                }
                //（4）调用详情
                return this.findDuePayApproveInit(payId);
            })
            .then(response => {
                //（5）设置详情数据
                if (response && response.code === 0 && response.data) {

                    this.payHisEntity = response.data.payHisEntity; 

                    this.loanDueEntity = response.data.loanDueEntity;
                        ...
                        api.findByDueIdAll(
                            {
                                ...
                                dynamicQuery: "",
                                sort: "[]",
                                tableId: "loan/loanPrcpRepayPlanPayShowTable",
                                initQuery:"{\"dueId\":\"" + this.payId + "\"}"
                            },
                            this.$config,
                            response => {
